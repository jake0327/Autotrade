...
def get_target_price(ticker, term):     # 매수 목표가 조회
    ...
    for k in np.arange(0.1, 1.0, 0.1):
         ...
         # ror = df['ror'][0] * 0.25 + df['ror'][1] * 0.2 + df['ror'][2] * 0.15 + df['ror'][3] * 0.1 + df['ror'][4] * 0.05 + df['ror'][5] * 0.05 + df['ror'][6] * 0.05 + df['ror'][7] * 0.05+ df['ror'][8] * 0.05 + df['ror'][9] * 0.05
         ror = df['ror'][0] * 0.4 + df['ror'][1] * 0.3 + df['ror'][2] * 0.2 + df['ror'][3] * 0.1
         if ror >= ava_ror :
             ava_ror = ror
             real_k = k
    target_price = np.where(ava_ror > 1.001, (df.iloc[0]['close']+(df.iloc[0]['high'] - df.iloc[0]['low']) * real_k), 1)
    return target_price
    ...
    while True:
        try:
            ...
            if start < now < end:
                ...
                if target_price != 1 and target_price < current_price and nowCoin < 0.0001 :
