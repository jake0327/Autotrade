...
def get_target_price(ticker, term):     # 매수 목표가 조회
    ...
    for k in np.arange(0.1, 1.0, 0.1):
         ...
         if ror >= ava_ror :
             ava_ror = ror
             real_k = k
    target_price = np.where(ava_ror > 1.1, (df.iloc[0]['close']+(df.iloc[0]['high'] - df.iloc[0]['low']) * real_k), 1)
    return target_price
    ...
    while True:
        try:
            ...
            if start < now < end:
                ...
                if target_price < current_price and target_price !=1 and nowCoin < 0.0001 :
