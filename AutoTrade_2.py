...
# 자동매매(1600 ~ 2000)
    while True:
        try:
            now = datetime.datetime.now()
            start = get_start('160000')
            end = get_end('190000')
            end2 = get_end('215900')
            nowCoin = upbit.get_balance("KRW-BTC")
            if start < now < end:
                print("자동매매(1600 ~ 1900) 시작")
                target_price = get_target_price('KRW-BTC', "minute60")
                current_price = get_current_price("KRW-BTC")
                if target_price < current_price and nowCoin < 0.0001 :
                    print('매수 진행 중(1600 ~ 1900)')
                    krw = upbit.get_balance("KRW")
                    value = krw * 0.9995
                    if value > 5000:
                        print('매수 주문(1600 ~ 1900)')
                        buy_result = upbit.buy_market_order("KRW-BTC", value)
                        print(buy_result)
                        if buy_result == None or 'error' in buy_result:
                            print("매수 재주문(1600 ~ 1900)")
                            time.sleep(1)
                    else:
                        print("잔고 부족(1600 ~ 1900)")
            elif end < now < end2 and nowCoin != 0:
                print("매도 처리 중(1600 ~ 1900)")
                BTC = upbit.get_balance("KRW-BTC")
                upbit.sell_market_order("KRW-BTC", BTC)
        except:
            print("1600 ~ 1900 종료")        
...
th_1600 = threading.Thread(target=time1600, args=())
