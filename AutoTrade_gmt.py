...
access ="CjegqVwogjQ5awAAP20WDE4fEpbxxvjlcy4zEfx4"
secret = "TLDa85Tb3Xlk1rK09BUDy3xxUn6xsAd1rpxwGghe"
...
# 자동매매(0900 ~ 1300)
    while True:
        try:
            now = datetime.datetime.now()
            start = get_start_gmt('090000')
            end = get_end_gmt('130000')
            end2 = get_end_gmt('162900')
            nowCoin = upbit.get_balance("KRW-BTC")
            if start < now < end:
                print("자동매매(0900 ~ 1300) 중")
...
# 자동매매(1630 ~ 1930)
    while True:
        try:
            now = datetime.datetime.now()
            start = get_start_gmt('163000')
            end = get_end_gmt('193000')
            end2 = get_end_gmt('215900')
            nowCoin = upbit.get_balance("KRW-BTC")
            if start < now < end:
                print("자동매매(1630 ~ 1930) 중")
...
def time2200() :
    ...
    while True:
        try:
            now = datetime.datetime.now()
            start = get_start_gmt('220000')
            end = get_end_gmt('235930')
            nowCoin = upbit.get_balance("KRW-BTC")
            if start < now < end:
                print("자동매매(2200 ~ 0000) 중")
                target_price = get_target_price('KRW-BTC', "minute60")
                current_price = get_current_price("KRW-BTC")
                if target_price < current_price and nowCoin < 0.0001 :
            ...
            elif end < now < end + datetime.timedelta(hours=17) and nowCoin != 0:
